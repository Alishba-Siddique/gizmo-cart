...
const StoreInfo = ({ store }) => {
  return (
    <div className="flex-1 space-y-2 text-sm">
      ...
      <p className="text-zinc-600 my-5 max-w-2xl">{store.description}</p>
      ...
      <div className="flex items-center gap-2 text-sm ">
        ...
        />
        <div>
          <p className="text-zinc-600 font-medium">{store.user.name}</p>
          <p className="text-zinc-400">{store.user.email}</p>
        </div>
      </div>
    </div>
  );
